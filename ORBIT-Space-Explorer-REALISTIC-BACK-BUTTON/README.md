...
- **Realistic 3D bodies** — every planet, the Sun, and any body you search
  for gets a real-looking surface: continent-like terrain for rocky bodies,
  turbulent bands for gas giants, a churning fiery surface for the Sun,
  drifting clouds on Earth, and a textured ring for Saturn — plus a matching
  relief (bump) map so the surface actually looks like it has depth under
  lighting. All of it is generated procedurally at runtime (see
  `src/utils/proceduralTextures.js`), so there are no external image files
  to break or fail to load.
...
└── src/
    ...
    ├── utils/
    │   └── proceduralTextures.js # Runtime-generated realistic surfaces (no image assets)
    └── components/
...
## Realism upgrade
- High-resolution procedural 1024px planet textures with body-specific profiles.
- Earth cloud shell and stronger atmospheric scattering-style glow.
- More detailed rocky relief and gas-giant banding.
- Improved Saturn ring density and geometry.
- Procedural Milky-Way-style spiral galaxy, nebula cloud, asteroid belt and black-hole showcase.
- Sun now uses a high-resolution turbulent photosphere, emissive corona and scene illumination.

The models are **scientifically inspired visual approximations**, not metrically accurate digital twins. For true surface accuracy, downloadable NASA/ESA imagery or mission-derived 3D meshes can be added later as optional assets.
