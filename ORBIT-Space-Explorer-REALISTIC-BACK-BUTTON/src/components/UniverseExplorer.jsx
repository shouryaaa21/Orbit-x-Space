...
export default function UniverseExplorer({ open, onClose }) {
  ...
  return (
    <div className="universe-overlay">
      {viewingImage ? (
        <div className="universe-image-viewer">
          <button className="image-back" onClick={() => setViewingImage(null)}>
            ← Back to {result?.name || 'object'}
          </button>
          ...
          <div className="image-viewer-caption">
            <div>
              <span className="kicker">SPACE AGENCY IMAGE</span>
              <h2>{viewingImage.title}</h2>
            </div>
            <a href={viewingImage.url} target="_blank" rel="noreferrer">Open original ↗</a>
          </div>
        </div>
      ) : (
